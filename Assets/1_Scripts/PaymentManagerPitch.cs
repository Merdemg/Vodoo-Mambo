...
public class PaymentManagerPitch : MonoBehaviour {
	...
	public static string[] CoinIdentifiers {
		get {
			return new string[]{ coins1, coins2, coins3, coins4 };
		}
	}
    public const string coins1 = "com.sillywalk.pitchpincher.coins.1";
    public const string coins2 = "com.sillywalk.pitchpincher.coins.2";
    public const string coins3 = "com.sillywalk.pitchpincher.coins.3";
    public const string coins4 = "com.sillywalk.pitchpincher.coins.4";
    public static int[] coinValues = new int[]{ 100, 500, 1000, 5000 };
...
//	public string lastTransactionProdudctId = string.Empty;
	...
	public static System.Action<bool> nextTransactionCallback = null;
...
//			PaymentManager.Instance.AddProductId(NC_PACK);
	...
	public static void BuyItem(string productId, System.Action<bool> onTransactionDone) {
		PaymentManager.Instance.BuyProduct(productId);
//		UIManager.Instance.SetShowLoading (true);

		nextTransactionCallback = onTransactionDone;
	}
...
//			UnlockProducts (result.ProductIdentifier);
			if(nextTransactionCallback != null) {
				nextTransactionCallback (true);
				nextTransactionCallback = null;
			}
...
//		IOSNativePopUpManager.showMessage("Verification", "Transaction verification status: " + response.Status.ToString());
	...
	private static void OnStoreKitInitComplete(SA.Common.Models.Result result) {
		...
		if(result.IsSucceeded) {
            ...
            UIManager.Instance.shopPanel.SetItemCosts(PaymentManager.Instance.Products.ToArray());
    //        foreach (var item in PaymentManager.Instance.Products)
    //        {
                
    //            ISN_Logger.Log("_______");
    //            ISN_Logger.Log("item id: " + item.Id);
				//ISN_Logger.Log("displayName: " + item.DisplayName);
            //    ISN_Logger.Log("price: " + item.LocalizedPrice);
            //    ISN_Logger.Log("currency: " + item.CurrencySymbol);
            //    ISN_Logger.Log("currency code: " + item.CurrencyCode);
            //    ISN_Logger.Log("currency code: " + item.Description);
            //    //ISN_Logger.Log("currency code: " + item.);
            //}
